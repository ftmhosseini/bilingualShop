    ...
    <div style={{ background: '#f9f9f9', border: '1px solid #ddd', borderRadius: 8, padding: 16, marginBottom: 12 }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        ...
        <div className="form-group" style={{ display: 'flex', alignItems: 'center', gap: 8, paddingTop: 24 }}>
          ...
          <label htmlFor="is_default" style={{ marginBottom: 0 }}>Set as default</label>
        </div>
      </div>
...
export default function Profile() {
  ...
  const [editingAddr, setEditingAddr] = useState(null); // null | 'new' | address object
  ...
  const submit = async e => {
    e.preventDefault();
    setError('');
    if (form.new_password && form.new_password !== form.confirm_password) { setError('New passwords do not match'); return; }
    try {
    ...
    <div className="page" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="profile-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, alignItems: 'start' }}>
        ...
        <form className="card" onSubmit={submit}>
          ...
          <p style={{ fontSize: 13, color: '#888', marginBottom: 12 }}>Leave password fields empty to keep current password.</p>
          <div className="form-group">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            {saved === 'profile' && <span style={{ color: 'green', fontSize: 13 }}>✓ Saved</span>}
          </div>
        </form>
        ...
        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            {editingAddr === null && <button className="btn btn-secondary" onClick={() => setEditingAddr('new')}>+ Add</button>}
          </div>

          {editingAddr === 'new' && <AddressForm onSave={saveAddress} onCancel={() => setEditingAddr(null)} />}

          {addresses.map(a => (
            <div key={a.id}>
              {editingAddr?.id === a.id ? (
                <AddressForm initial={a} onSave={saveAddress} onCancel={() => setEditingAddr(null)} />
              ) : (
                <div style={{ border: a.is_default ? '2px solid #febd69' : '1px solid #ddd', borderRadius: 8, padding: 12, marginBottom: 10, background: a.is_default ? '#fffbe6' : '#fff' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                      ...
                      {a.is_default ? <span style={{ marginLeft: 8, fontSize: 11, background: '#febd69', padding: '2px 6px', borderRadius: 4 }}>Default</span> : null}
                    ...
                    <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                      {!a.is_default && <button className="btn btn-secondary" style={{ fontSize: 12, padding: '4px 8px' }} onClick={() => setDefault(a.id)}>Default</button>}
                      <button className="btn btn-secondary" style={{ fontSize: 12, padding: '4px 8px' }} onClick={() => setEditingAddr(a)}>Edit</button>
          ...
          {addresses.length === 0 && editingAddr === null && <p style={{ color: '#888', fontSize: 13 }}>No saved addresses yet.</p>}
        </div>
      </div>
    </div>
  );
